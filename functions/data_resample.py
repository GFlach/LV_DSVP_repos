...
def resample(in_name, out_name, up=1, down=4, f_neu=11025):
    # Read input wave
    path_name = 'sound\\'
    file_name_in = path_name + in_name + '.wav'
    file_name_out = path_name + out_name + '.wav'
    in_fr, in_data = wavfile.read(file_name_in)
    ...
    # Filter Design
    L = up         # Upsampling Factor
    M = down         # Downsampling Factor
    Nz = int(25)  # Max Number of Zero Crossings (depending on quality)
...
def resample_file(in_data):
    ...
    # Write the output wave
    file_name_out = 'C:\\WinPython\\notebooks\\sound\\mischton_800.wav'
    wavfile.write(file_name_out, out_fr, out_data)
    
    # Print Results
    print ("File", file_name_out, "was successfully written")
    #print ("It took: %.2f seconds" % float(tm.time() - start_time))

#resample('dsv1_traktor_fahrt.wav', 4, 1, 44100, 'dsv1_traktor_fahrt_44100.wav')